...
filters += ‘&itemFilter(’ + (i+2) + ‘).name=Condition&itemFilter(’ + (i+2) + ‘).value(’ + i + ‘)=’ + COND_NAMES[condIds[i]];
}
var path = ‘/services/search/FindingService/v1?OPERATION-NAME=findCompletedItems&SERVICE-VERSION=1.0.0&SECURITY-APPNAME=’ + APP_ID + ‘&RESPONSE-DATA-FORMAT=JSON&REST-PAYLOAD&keywords=’ + encodeURIComponent(keywords) + ‘&categoryId=267&itemFilter(0).name=SoldItemsOnly&itemFilter(0).value=true&itemFilter(1).name=ListingType&itemFilter(1).value=FixedPrice’ + filters + ‘&paginationInput.entriesPerPage=20&sortOrder=EndTimeSoonest’;
var opts = {
...
if (resp[0].ack[0] !== ‘Success’) { cb(null, ‘API error’); return; }
var items = resp[0].searchResult && resp[0].searchResult[0] && resp[0].searchResult[0].item;
if (!items || items.length === 0) { cb(null, ‘No items’); return; }
var prices = [];
for (var i = 0; i < items.length; i++) {
var ss = items[i].sellingStatus && items[i].sellingStatus[0];
var p = ss && ss.convertedCurrentPrice && ss.convertedCurrentPrice[0] && parseFloat(ss.convertedCurrentPrice[0][’__value__’]);
...
if (!title && !isbn) { res.writeHead(400); res.end(’{“error”:“missing title”}’); return; }
var kw = isbn ? isbn : (title + (author ? ’ ’ + author : ‘’) + (signed ? ’ signed’ : ‘’));
console.log(‘Search:’, kw, ‘Cond:’, condIds.join(’,’));
...
res.writeHead(200);
res.end(JSON.stringify(err ? { error: err, average: null, count: 0 } : result));
});
});
